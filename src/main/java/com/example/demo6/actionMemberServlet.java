...
@WebServlet(name = "actionMemberServlet", value = "/actionMember-servlet")
public class actionMemberServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("<br><html lang='ru'><div style='text-align: center;'><head><title>Data base</title><meta http-equiv='Content-Type' content='text/html; charset=utf-8'></head><body><form action='actionMember-servlet' method='POST'>" +
                "<h2>Имя:<input name='name' /><br><br>Фамилия:<input name='second_name' /><br><br>Отчество:<input name='patronymic' /><br><br>" +
                "Возраст:<input name='age' /><br><br>Роль:<input name='role' /><br><br><font size='24pt'><input type='submit' name='s1' value='Подтвердить' /></font>" +
                "</h2></form>");
        try {
            ...
            out.println("<form action='actionMember-servlet' method='POST'>");
            out.println("<tr><td>Id</td><td>Имя</td><td>Фамилия</td><td>Отчество</td><td>Возраст</td><td>Роль</td></tr>");
            int i=0;
            while(resultSet.next()){
                ...
                out.println("<tr><td>"+i+"</td><td>"+name+"</td><td>"+second_name+"</td><td>"+patronymic+"</td><td>"+age+"</td><td>"+role+"</td></tr>");
            }
            resultSet.close();
            st.close();
            conn.close();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(cap.equals("Подтвердить")){
            ...
            String patronymic1 = request.getParameter("patronymic");
            String age1 = request.getParameter("age");
            String role1 = request.getParameter("role");
            String af =String.format("UPDATE public.member SET id='"+lap+"', first_name='"+name1+"', second_name='"+second_name1+"', patronymic='"+patronymic1+"', age='"+age1+"', role='"+role1+"' WHERE id='"+lap+"'");
            try {
                st.executeUpdate(af);
            } catch (SQLException e) {
                throw new RuntimeException(e);
            ...
            try {
                ...
                while(resultSet.next()){
                    ...
                    String age=resultSet.getString(5);
                    String role=resultSet.getString(6);
                    out.println("<tr><td>"+i+"</td><td>"+name+"</td><td>"+second_name+"</td><td>"+patronymic+"</td><td>"+age+"</td><td>"+role+"</td><td><a href='actionMember-servlet?id="+id+"'>Изменить</a>|<a href='delete_member.jsp?id="+id+"'>Удалить</a></td></tr>");
                }
                resultSet.close();
                st.close();
                conn.close();
